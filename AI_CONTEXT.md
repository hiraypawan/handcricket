...
| **Quick matchmaking** | Seekers poll `POST /api/quickmatch` every 2s; same-format pairs match instantly (earlier seeker hosts, room code shared). Rooms are deterministic per pair so concurrent polls converge instead of stranding the guest; an existing record for the pair is adopted, never duplicated. Nothing by a random 18–25s cutoff → bot persona fallback (a Play Bot Now shortcut appears after ~8s). Waiting responses carry the live seeker count. Cancel/Start leaves the pool; matched pairs ping `joined` (anonymous `quick:events` ring buffer tracks seek/match/join health). `window.__qmCutoffMs` overrides the cutoff in tests. | `functions/api/quickmatch.js`, `18-instant.js` |
